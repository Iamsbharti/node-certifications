...
async function writer() {
  ...
  console.log("test file val::", file, answer);
...
function exercise(project) {
  ...
  fs.watch(project, (evt, filename) => {
    try {
      ...
      const { ctimeMs, mtimeMs } = stat;
      // TODO - only set the answer variable if the filepath
      // in both newly created AND does not point to a directory
      console.log(ctime, mtime, filepath, stat.isDirectory());
